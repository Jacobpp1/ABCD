...
class main{
    public static void Main(){
        ...
        + $" should be 2±{delta}=[{lims[0]},{lims[1]}]" + $" which is: " + $"{lim_check(lims, res)}");
        //WriteLine("1/sqrt(x) evals: " + evals);

        res = adapt_int(x => Log(x)/Sqrt(x), 0, 1, delta, counter:0);
        lims[0] = res-delta; lims[1] = res+delta;
    ...
    double delta=0.001, double eps=0.001, double f2=Double.NaN, double f3=Double.NaN, int counter = -1){
        if(counter != -1){
            counter++;
            WriteLine(counter);
        }
        ...
        if (err <= delta+eps*Abs(Q)) return Q;//(Q, counter);
            ...
            return (integrate(f,a,(a+b)/2,delta/Sqrt(2),eps,f1,f2, counter)+
                    integrate(f,(a+b)/2,b,delta/Sqrt(2),eps,f3,f4, counter));
    ...
    double delta=0.001, double eps=0.001, double f2=Double.NaN, double f3=Double.NaN, int counter = -1){
        //if(counter != -1)
        //    counter++;
        return integrate(x => f((a+b)/2+(b-a)/2*Cos(x))*Sin(x)*(b-a)/2, 0, PI, delta, eps, f2, f3, counter);
    }
