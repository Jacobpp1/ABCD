...
When saying variance is used as error, I refer to the error being V*sigma/sqrt(N).
And when the error is the difference in mean, it is the absolute value of the mean from the previous iteration minus the current one, and multiplied by b-a.
The Clenshaw-Curtis transformation is referred to as CC-transform(ation) in the Out.txt file.
